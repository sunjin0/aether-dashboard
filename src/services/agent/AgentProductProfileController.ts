...
export interface AgentProductProfile { id: string; productId: string; applicationId: string; code: string; agentDefinitionId?: string; workflowId?: string; productType: string; name: string; inputSchema: string; outputSchema: string; knowledgePolicy?: string; approvalPolicy?: string; handoffPolicy?: string; apiProtocolVersion?: string; allowedContextKeys?: string; publishedSnapshotId?: string; status: number; versionNo: number; creatorUserId?: string }
export const getAgentProductProfiles = (data: { current?: number; pageSize?: number; applicationId?: string; name?: string; productType?: string; status?: number; creatorUserId?: string } = {}) => request<ResponseStructure<AgentProductProfile[]>>('/api/agent/product-profile/list', { method: 'POST', data })
