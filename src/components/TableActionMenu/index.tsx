...
const TableActionMenu: React.FC<TableActionMenuProps> = ({ items, maxPrimary = 3 }) => {
  ...
  const menuItems: MenuProps['items'] = moreItems.map((item) => ({
    ...
    danger: item.danger,
    label: item.confirm ? (
      <Popconfirm title={item.confirm.title} description={item.confirm.description} onConfirm={item.onClick}>
  ...
  return (
    <Space size={0} wrap>
      {primaryItems.map((item) => item.confirm ? (
        <Popconfirm key={item.key} title={item.confirm.title} description={item.confirm.description} onConfirm={item.onClick}>
          <Button type="link" danger={item.danger} loading={item.loading}>{item.label}</Button>
        </Popconfirm>
      ) : (
        <Button key={item.key} type="link" danger={item.danger} loading={item.loading} onClick={item.onClick}>{item.label}</Button>
      ))}
      {moreItems.length > 0 && (
