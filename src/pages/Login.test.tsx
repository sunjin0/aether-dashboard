...
jest.mock('@ant-design/pro-components', () => {
  const React = require('react')
  const ProFormText = ({ name, initialValue }: any) => (
    <div data-testid={`${name}-field`}>{initialValue}</div>
  )
  ...
  return {
    ...
    LoginForm: ({ children, title, subTitle, onFinish }: any) => (
      <form>
        ...
        <p>{subTitle}</p>
        {children}
        <button type="button" onClick={() => onFinish({ account: 'admin', password: 'password' })}>
          submit
        </button>
      </form>
    ),
...
describe('Login', () => {
  ...
  it('keeps nested inputs transparent so the light field background remains visible while focused', () => {
    ...
    expect(styles).toMatch(
      /\.ant-input-affix-wrapper\s*\{[\s\S]*?\.ant-input\s*\{\s*background: transparent;/,
    )
  })
  ...
  it('keeps validation-error inputs on the light field background', () => {
    ...
    expect(styles).toMatch(
      /\.ant-input-status-error,[\s\S]*?\.ant-input-affix-wrapper-status-error\s*\{[\s\S]*?background: #fff !important;/,
    )
  })
  ...
  it('uses dark high-contrast colors for clear and password visibility icons', () => {
    ...
    expect(styles).toMatch(
      /\.ant-input-clear-icon,[\s\S]*?\.ant-input-password-icon\s*\{[\s\S]*?color: rgba\(30, 50, 82, 0\.72\) !important;/,
    )
  })
  ...
  it('uses a light page surface and white login panel', () => {
    ...
    expect(styles).toMatch(
      /\.login-page\s*\{[\s\S]*?color: #172b4d;[\s\S]*?linear-gradient\(135deg, #f7fbff 0%, #edf5ff 48%, #f7f2ff 100%\)/,
    )
  ...
  it('updates the current user before redirecting successful logins to the dashboard', async () => {
    ...
    expect(mockSetInitialState.mock.invocationCallOrder[0]).toBeLessThan(
      mockHistoryPush.mock.invocationCallOrder[0],
    )
  })
})
