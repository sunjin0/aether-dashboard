    ...
    <ProConfigProvider hashed={false}>
      <section className="login-page" data-testid="login-page">
        <aside className="login-brand" aria-label="Aether">
          ...
          <h1>
            {formatMessage('user.login.brand.headline.before')}{' '}
            <span>{formatMessage('user.login.brand.headline')}</span>
          </h1>
        ...
        <main className="login-panel-wrap">
          <div className="login-panel">
            <LoginForm
              submitter={{
                searchConfig: {
                  submitText: formatMessage(isEmailStep ? 'user.login.submit' : 'user.login.next'),
                },
            ...
            >
              <div className="login-stage-label">
                {formatMessage(
                  isEmailStep ? 'user.login.email.eyebrow' : 'user.login.account.eyebrow',
                )}
              </div>
              <div className="login-stage-header">
                <div>
                  <h2>
                    {formatMessage(
                      isEmailStep ? 'user.login.email.title' : 'user.login.account.title',
                    )}
                  </h2>
                  <p>
                    {formatMessage(
                      isEmailStep
                        ? 'user.login.email.description'
                        : 'user.login.account.description',
                    )}
                  </p>
                </div>
                <div
                  ...
                  aria-label={formatMessage(
                    isEmailStep ? 'user.login.step.email' : 'user.login.step.account',
                  )}
                >
                  <span className="is-active" />
                  <span className={isEmailStep ? 'is-active' : ''} />
                </div>
              </div>
              {!isEmailStep && (
                <>
