...
describe('ReviewIssueList', () => {
  it('shows issue actions only after the issue is selected', () => {
    ...
    expect(screen.queryByRole('button', { name: /pages\.knowledge\.review\.issueList\.accept/ })).toBeNull()
    fireEvent.click(screen.getByText(issue.message))
    expect(screen.getByRole('button', { name: /pages\.knowledge\.review\.issueList\.accept/ })).toBeTruthy()
    expect(screen.getByRole('button', { name: /pages\.knowledge\.review\.issueList\.reject/ })).toBeTruthy()
  })
})
