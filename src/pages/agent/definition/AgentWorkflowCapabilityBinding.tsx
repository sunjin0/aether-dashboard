...
import { Button, Card, Col, Empty, Input, Modal, Row, Spin, Switch, Tabs, Tag, Tooltip, Typography } from 'antd'
...
const AgentWorkflowCapabilityBinding: React.FC<Props> = ({ agentId, open, setOpen }) => {
  ...
  return <Modal title={text('pages.agent.workflowCapability.binding.title')} open={open} onCancel={() => setOpen(false)} footer={null} width="min(980px, calc(100vw - 24px))" destroyOnClose>
    ...
    <Spin spinning={loading}><Row gutter={[12, 12]}>{items.length ? items.map((item: any) => <Col xs={24} sm={12} lg={8} key={item.id || item.capabilityId}><Card size="small" title={<Typography.Text ellipsis={{ tooltip: item.displayName }}>{item.displayName || item.capabilityCode}</Typography.Text>} extra={item.riskLevel && <Tag color={item.riskLevel === 'HIGH' ? 'red' : item.riskLevel === 'MEDIUM' ? 'orange' : 'green'}>{text(`pages.agent.workflowCapability.risk.${item.riskLevel}`)}</Tag>}><Typography.Paragraph ellipsis={{ rows: 2 }}>{item.description || item.capabilityCode}</Typography.Paragraph><div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>{bound ? <><Switch size="small" checked={item.status === 1} onChange={status => item.capabilityId && updateWorkflowCapabilityBindingStatus(agentId, item.capabilityId, status ? 1 : 0).then(() => load())} /><Tooltip title={text('pages.common.delete')}><Button size="small" type="text" danger icon={<DeleteOutlined />} onClick={() => toggle(item)} /></Tooltip></> : <Button size="small" type="primary" icon={<PlusOutlined />} onClick={() => toggle(item)}>{text('pages.agent.skill.install')}</Button>}</div></Card></Col>) : <Col span={24}><Empty description={text(bound ? 'pages.agent.definition.noBoundItems' : 'pages.agent.definition.noAvailableItems')} /></Col>}</Row></Spin>
  </Modal>
}
