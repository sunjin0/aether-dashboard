...
import { Document, DocumentSearchParams, KnowledgeDocumentChunk, KnowledgeDocumentVersion } from '@/services/entity/Agent'
import { ResponseStructure } from '@/services/entity/Common'

export const getDocumentList = async (params: DocumentSearchParams): Promise<ResponseStructure<Document[]>> =>
...
export const uploadDocument = async (knowledgeBaseId: string, file: File, title?: string): Promise<ResponseStructure<string>> => {
...
export const getDocumentVersions = async (id: string): Promise<ResponseStructure<KnowledgeDocumentVersion[]>> =>
  request(`/api/knowledge/document/${id}/versions`, { method: 'GET' })
export const rollbackDocumentVersion = async (versionId: string): Promise<ResponseStructure<string>> =>
  request(`/api/knowledge/document/version/${versionId}/rollback`, { method: 'POST' })
/** 查询指定文档版本按 chunkNo 升序返回的分块列表。 */
export const getDocumentVersionChunkList = async (versionId: string): Promise<ResponseStructure<KnowledgeDocumentChunk[]>> =>
