...
import { AgentProductProfile, AgentProductProfileVersion, copyAgentProductProfile, createAgentProductProfile, getAgentProductProfileVersions, getAgentProductProfiles, publishAgentProductProfile, updateAgentProductProfile } from '@/services/agent/AgentProductProfileController'
...
export default function AgentProductProfilePage() {
 const ref = useRef<any>(); const [open, setOpen] = useState(false); const [current, setCurrent] = useState<AgentProductProfile>(); const [apps, setApps] = useState<AgentApplication[]>([]); const [agents, setAgents] = useState<any[]>([]); const [versions, setVersions] = useState<AgentProductProfileVersion[]>([]); const [versionOpen, setVersionOpen] = useState(false)
 ...
 return <PageContainer header={{ title: 'Agent 产品发布', subTitle: '以产品契约交付智能客服、智能问答与业务助手' }}>
  <ProTable<AgentProductProfile> actionRef={ref} rowKey="id" search={false} request={async () => { const r = await getAgentProductProfiles(); return { data: r.data || [], success: r.code === 200 } }} toolBarRender={() => [<Button key="new" type="primary" icon={<PlusOutlined />} onClick={() => { setCurrent(undefined); setOpen(true) }}>新建产品</Button>]}
   columns={[{ title: '名称', dataIndex: 'name' }, { title: '类型', dataIndex: 'productType', valueEnum: { CUSTOMER_SERVICE: '智能客服', KNOWLEDGE_QA: '智能问答', BUSINESS_ASSISTANT: '业务助手' } }, { title: '版本', dataIndex: 'versionNo', render: (_, r) => r.status === 1 ? <Tag color="green">已发布 v{r.versionNo}</Tag> : <Tag>待发布草稿</Tag> }, { title: '操作', valueType: 'option', render: (_, r) => [r.status !== 1 && <a key="edit" onClick={() => { setCurrent(r); setOpen(true) }}>编辑</a>, r.status !== 1 && <a key="publish" onClick={async () => { const result = await publishAgentProductProfile(r.id); if (result.code === 200) ref.current?.reload() }}>发布</a>, r.status === 1 && <a key="copy" onClick={async () => { const result = await copyAgentProductProfile(r.id); if (result.code === 200) ref.current?.reload() }}>创建新草稿</a>, r.status === 1 && <a key="versions" onClick={async () => { const result = await getAgentProductProfileVersions(r.id); if (result.code === 200) { setVersions(result.data || []); setVersionOpen(true) } }}>版本历史</a>] }]} />
  ...
  <Modal open={versionOpen} onCancel={() => setVersionOpen(false)} footer={null} title="已发布版本历史" width={760}>
   <Table rowKey="id" pagination={false} dataSource={versions} columns={[{ title: '版本', dataIndex: 'versionNo', render: (v) => `v${v}` }, { title: '发布时间', dataIndex: 'publishedAt', render: (v) => v ? new Date(v).toLocaleString() : '-' }, { title: '发布人', dataIndex: 'publishedBy', render: (v) => v || '-' }, { title: '配置快照', dataIndex: 'snapshot', ellipsis: true }]} />
  </Modal>
 </PageContainer>
}
