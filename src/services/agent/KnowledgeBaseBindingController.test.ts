...
describe('KnowledgeBaseBindingController', () => {
  ...
  it('uses documented agent knowledge-base binding endpoints', async () => {
    ...
    await addKnowledgeBaseBinding({ agentDefinitionId: 'agent-1', knowledgeBaseId: 'knowledge-base-1', status: 1 });
    ...
    expect(mockedRequest).toHaveBeenNthCalledWith(3, '/api/agent/knowledge-base-binding/binding-1/status', {
      method: 'PUT',
      data: { status: 0 },
    });
    expect(mockedRequest).toHaveBeenNthCalledWith(4, '/api/agent/knowledge-base-binding/binding-1', {
      method: 'DELETE',
    });
  });
});
