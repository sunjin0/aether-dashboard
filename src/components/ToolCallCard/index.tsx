...
const ToolCallCard: React.FC<ToolCallCardProps> = ({ log, compact }) => {
  ...
  const toolStatusMap: Record<number, { text: string; color: string }> = {
    0: {
      text: intl.formatMessage({ id: 'components.toolCallCard.status.success' }),
      color: 'success',
    },
    1: {
      text: intl.formatMessage({ id: 'components.toolCallCard.status.failed' }),
      color: 'error',
    },
    2: {
      text: intl.formatMessage({ id: 'components.toolCallCard.status.timeout' }),
      color: 'warning',
    },
    3: {
      text: intl.formatMessage({ id: 'components.toolCallCard.status.blocked' }),
      color: 'purple',
    },
  };
  const statusInfo = toolStatusMap[log.status || 0] || {
    text: intl.formatMessage({ id: 'components.toolCallCard.status.unknown' }),
    color: 'default',
  };
  ...
  const header = (
    <div className="tool-call-card-header">
      ...
      <span className="tool-call-card-name">
        {log.toolName || intl.formatMessage({ id: 'components.toolCallCard.unknownTool' })}
      </span>
      ...
      children: (
        <div className="tool-call-card-detail">
          {log.arguments && (
            <div className="tool-call-card-section">
              <div className="tool-call-card-label">
                {intl.formatMessage({ id: 'components.toolCallCard.arguments' })}
              </div>
          ...
          {log.responseBody && (
            <div className="tool-call-card-section">
              <div className="tool-call-card-label">
                {intl.formatMessage({ id: 'components.toolCallCard.result' })}
              </div>
          ...
          {log.errorMsg && (
            <div className="tool-call-card-section">
              <div className="tool-call-card-label tool-call-card-label-error">
                {intl.formatMessage({ id: 'components.toolCallCard.error' })}
              </div>
