...
jest.mock('@monaco-editor/react', () => ({
  Editor: ({ value, onChange }: { value?: string; onChange?: (value?: string) => void }) => (
    <textarea aria-label="document-editor" value={value} onChange={(event) => onChange?.(event.target.value)} />
  ),
}))
...
describe('ReviewTaskDrawer', () => {
  beforeEach(() => {
    ...
    mockUpdateDocumentDraft.mockResolvedValue({ data: { id: 'version-1', content: '修改后正文', contentChecksum: 'checksum-2' } })
  })
  ...
  it('saves edited version content before approval', async () => {
    ...
    fireEvent.change(await screen.findByLabelText('document-editor'), { target: { value: '修改后正文' } })
    fireEvent.click(screen.getByRole('button', { name: 'pages.knowledge.review.detail.approve' }))

    await waitFor(() => expect(mockUpdateDocumentDraft).toHaveBeenCalledWith('version-1', '修改后正文', 'checksum-1'))
