...
export type WorkflowNode = { id: string; type: 'start' | 'agent' | 'mcp' | 'human' | 'end'; name?: string; resourceId?: string; prompt?: string; question?: string; toolName?: string; argumentsTemplate?: string; outputKey?: string; questions?: unknown[]; position?: { x: number; y: number } }
export type AgentWorkflow = { id?: string; name?: string; description?: string; nodes?: string; edges?: string; inputSchema?: string; status?: number; publishedVersion?: number; current?: number; pageSize?: number }
...
export const getWorkflowInstances = (data: WorkflowInstance) => request<ResponseStructure<WorkflowInstance[]>>('/api/agent/workflow/instances/list', { method: 'POST', data })
