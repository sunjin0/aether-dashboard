...
export type WorkflowOutputMapping = { target?: string; source?: string; template?: string; value?: unknown }
export type WorkflowNode = { id: string; type: 'start' | 'agent' | 'tool' | 'interaction' | 'rule' | 'http' | 'notification' | 'subflow' | 'parallel' | 'join' | 'wait_event' | 'delay' | 'end'; name?: string; resourceId?: string; prompt?: string; question?: string; toolName?: string; argumentsTemplate?: string; toolApprovalPolicy?: string; mode?: 'form' | 'approval'; outputs?: WorkflowOutputMapping[]; questions?: WorkflowQuestion[]; rules?: { condition?: string; value?: unknown }[]; defaultValue?: unknown; url?: string; method?: string; bodyTemplate?: string; idempotencyKeyTemplate?: string; channel?: string; toTemplate?: string; subjectTemplate?: string; workflowId?: string; versionNo?: number; inputMappings?: { target?: string; source?: string }[]; eventType?: string; correlationKeyTemplate?: string; timeoutMillis?: number; timeoutTargetId?: string; approverServiceAccountId?: string; approvalMode?: 'ANY' | 'ALL'; maxBranches?: number; branchTimeoutMillis?: number; delayMillis?: number; position?: { x: number; y: number }; [key: string]: unknown }
