  ...
  return (
    ...
    >
      <ProTable<KnowledgeDocumentChunk>
        ...
        search={false}
        pagination={false}
        request={() => version?.id ? getDocumentVersionChunkList(version.id) : Promise.resolve({ code: 200, data: [] })}
        columns={[
          { title: '序号', dataIndex: 'chunkNo', width: 90, valueType: 'digit' },
          {
            ...
            dataIndex: 'content',
            copyable: true,
            render: (_, record) => <Typography.Paragraph ellipsis={{ rows: 3, tooltip: record.content }}>{record.content || '-'}</Typography.Paragraph>,
          },
