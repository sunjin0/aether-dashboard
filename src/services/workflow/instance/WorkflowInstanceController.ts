...
export type WorkflowInstance = { id: string; workflowId: string; workflowName?: string; status: string; startedAt?: number; completedAt?: number; variables?: string; currentNodeId?: string; errorMessage?: string; versionNodes?: string; versionEdges?: string; nodes?: WorkflowNodeInstance[]; businessType?: string; businessId?: string; idempotencyKey?: string; deadlineAt?: number; current?: number; pageSize?: number; creatorUserId?: string; pendingSubflowInteraction?: WorkflowPendingSubflowInteraction }
export type WorkflowInstanceQuery = Partial<Pick<WorkflowInstance, 'workflowId' | 'status' | 'businessType' | 'businessId' | 'current' | 'pageSize' | 'creatorUserId'>>
