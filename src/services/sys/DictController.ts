 ...
 * @param params 查询参数
 */
export const getDictByParentCodeAndValue = async (
  params: DictSearchParams,
): Promise<ResponseStructure<Dict[]>> => {
 ...
 * @since 2025-07-17
 */
export const deleteDictInfo = async (
  params: DictSearchParams,
): Promise<ResponseStructure<Dict>> => {
...
export const getOptionList = async (
  ...
  const { data } = await request('/api/sys/dict/options', {
    method: 'GET',
    params: {
      parentCode,
      useValue,
